...
def log_posterior_probs(x):
    ...
    # Iterate over the 3 hypotheses
    for i in range(3):
        log_prior = np.log(PRIOR_PROBS[i])

        log_likelihood = np.sum((np.log([HYPOTHESIS_SPACE[i].pdf(x_i) for x_i in x])))

        # Store the unnormalized posterior
        log_p.append(log_prior + log_likelihood)

    # Normalize
    log_normalizer = logsumexp(log_p)
    log_p = log_p - log_normalizer
